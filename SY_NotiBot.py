...
def PrintBlobFiles(query_data, chatID, storage_name = 'smartwatchdata', ):
    ...
    bot.sendMessage(chatID, str(len(file_info)) + " videos from " + query_data + ":\n-------------------\n" + printMsg)
    return
...
def chat_handle(msg):
    ...
    if '20' in msg["text"] :
        PrintBlobFiles(msg["text"], chat_id)
    else:
        #SHOW BUTTONS
        ...
        bot.sendMessage(chat_id, "Choose a date!", reply_markup = mu)
...
def query_ans(msg):         #activated when btn is selected
    ...
    if query_data == "other":
        bot.sendMessage(from_id, "input the date in the form of yyyy-mm-dd")
        return
    ...
    PrintBlobFiles(query_data, from_id)
    return 
